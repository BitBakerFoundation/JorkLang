package org.typesystem.utils;

public interface TypeList {
    String INTEGER_OBJECT = "INTEGER";
    String FLOAT_OBJECT = "FLOAT";
    String STRING_OBJECT = "STRING";
    String BOOLEAN_OBJECT = "BOOLEAN";
    String ARRAY_OBJECT = "ARRAY";
    String HASH_OBJECT = "HASH";
    String NULL_OBJECT = "NULL";
    String RETURN_OBJECT = "RETURN";
    String ERROR_OBJECT = "ERROR";
    String FUNCTION_OBJECT = "FUNCTION";
    String BUILTIN_FUNCTION_OBJECT = "BUILTIN_FUNCTION";
    String COMPILED_FUNCTION_OBJECT = "COMPILED_FUNCTION";
    String COMPILED_BLOCK_OBJECT = "COMPILED_BLOCK";
    String CLOSURE_OBJECT = "CLOSURE";
}
